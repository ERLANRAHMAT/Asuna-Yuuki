...
const username = lann.message.from.username ? lann.message.from.username : "lann";
...
const getCase = (cases) => {
return "case"+`'${cases}'`+fs.readFileSync("./command/index.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
}
reply(`${getCase(q)}`)
...
const api = await fetchJson(`https://mfarels.my.id/api/openai?text=${text}`);
let lanntang = `${api.result}`;
reply(lanntang);
break

case 'ai-image': case 'openai-image': case 'aiimg':
if (!text) throw 'Masukkan text!';
let ini = global.api('lann', '/api/search/openai-image', {text: text}, 'apikey')
let aiimg = await fetch(ini)
if (!aiimg.ok) throw await aiimg.text()
lann.replyWithPhoto({
url: ini
}, {
caption: lang.ok,
